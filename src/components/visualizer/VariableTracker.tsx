...
export default function VariableTracker({ steps, currentStep }: VariableTrackerProps) {
  const vars = currentStep >= 0 ? collectVars(steps, currentStep) : {};
  const currentStepData = currentStep >= 0 ? steps[currentStep] : null;
  const changedKeys = new Set(
    currentStepData ? Object.entries(currentStepData.vars).filter(([, v]) => v.changed).map(([k]) => k) : []
  );
        ...
        <div className="space-y-2">
          <AnimatePresence mode="popLayout">
            {Object.entries(vars).map(([name, val]) => (
              ...
              >
                <span className="font-mono text-sm text-blue-300 shrink-0">{name}</span>
                <span className="text-zinc-600">=</span>
                <span className="font-mono text-sm text-green-300">{val.value}</span>
